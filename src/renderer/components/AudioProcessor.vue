...
function drawCurve() {
  ...
  if (!eqSettings.enabled) {
    ...
    ctx.strokeStyle = 'rgba(61,127,255,0.3)'
    ctx.lineWidth   = 1.5
    ctx.setLineDash([4, 4])
    ctx.beginPath(); ctx.moveTo(0, midY); ctx.lineTo(W, midY); ctx.stroke()
    ctx.setLineDash([])
  ...
  grad.addColorStop(0,   'rgba(61,127,255,0.35)')
  grad.addColorStop(0.5, 'rgba(61,127,255,0.08)')
...
.mb-param input[type=range]::-webkit-slider-thumb {
  ...
  box-shadow: 0 0 4px rgba(61,127,255,0.4);
}
.mb-param span {
